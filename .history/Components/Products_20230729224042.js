      ...
      { loading ? (<div className='flex justify-center items-center'>
         ...
         : (products.map((product) => (
          ...
          <div className='text-lg font-semibold relative'>
            <h3 className='absolute top-15'>{product.title}</h3>
            <p className='absolute top-10'>${product.price.toFixed(2)}</p>
            <p className='text-gray-300 absolute bottom-10'>{product.category}</p>
          </div>
      </div>
        )))
      }
